...
import { NODE_TYPES, WIRE_KINDS, saveWires, activeGraph, safe, ctx } from './state.js?v=0.15.0';
 ...
 */
export function plannedSaves(graph, results, live, { applySelect = (m) => m, wireHolds = () => true } = {}) {
    ...
    for (const w of wires) {
        ...
        const text = picked.map(m => m.content).filter(Boolean).join('\n\n').trim();
        if (!text) continue;
